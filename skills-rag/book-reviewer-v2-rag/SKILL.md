---
name: book-reviewer-v2-rag
description: RAG-aware variant of book-reviewer-v2. Enhanced lector — stable RV-NNN finding IDs, world/ memory integration, campaign-pending awareness, a protected author's-voice catalog, and delta-review/sign-off modes that avoid re-flagging resolved issues. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to book-reviewer-v2.
---

# book-reviewer-v2-rag

## What this is

A thin RAG-aware wrapper around `skills/book-reviewer-v2`. Behavior, inputs, outputs, and guarantees are identical to the base skill; the only change is that canon/world lookups (who/what/when facts from `world/` memory) are answered by querying the Book Writer RAG endpoint instead of reading whole `world/` files.

## How to run

1. Read `skills-rag/_shared/canon-lookup-protocol.md` for the full Canon Lookup Protocol.
2. Read `skills/book-reviewer-v2/SKILL.md` (the base skill) and follow it IN FULL.
...
**EMPHATIC: manuscript prose under review must be read from the chapter files IN FULL — the RAG must never substitute for reading the text being reviewed (delta-review and sign-off passes included).**
