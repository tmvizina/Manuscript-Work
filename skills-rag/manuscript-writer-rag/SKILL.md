...
description: RAG-aware variant of manuscript-writer. The writer — takes an editorial plan and triages every finding as Implement, Push back, or Suggest-only with reasoning, applying only accepted edits and emitting suggested rewrites to a sidecar for human approval. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to manuscript-writer.
---
...
A thin RAG-aware wrapper around `skills/manuscript-writer`. Behavior, inputs, outputs, and guarantees are identical to the base skill; the only change is that canon/world lookups (who/what/when facts from `world/` memory) are answered by querying the Book Writer RAG endpoint instead of reading whole `world/` files.

## How to run

1. Read `skills-rag/_shared/canon-lookup-protocol.md` for the full Canon Lookup Protocol.
2. Read `skills/manuscript-writer/SKILL.md` (the base skill) and follow it IN FULL.
...
**EMPHATIC: manuscript prose under edit must be read from the chapter files IN FULL — the RAG must never substitute for reading the text being worked on.**
