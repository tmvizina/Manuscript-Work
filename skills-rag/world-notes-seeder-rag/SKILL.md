---
name: world-notes-seeder-rag
description: RAG-aware variant of world-notes-seeder. The pre-pipeline canon intake — transcribes the author's handwritten/scanned/typed world notes faithfully, classifies facts into the world/ buckets with stable CHAR-NNN/THR-NNN IDs, and reconciles against existing canon without overwriting; gaps become TBD or campaign-pending, never invented. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to world-notes-seeder.
---

# world-notes-seeder-rag

## What this is

A thin RAG-aware wrapper around `skills/world-notes-seeder`. Behavior, inputs, outputs, and guarantees are identical to the base skill; the only change is that canon/world lookups (who/what/when facts from `world/` memory) are answered by querying the Book Writer RAG endpoint instead of reading whole `world/` files.

## How to run

1. Read `skills-rag/_shared/canon-lookup-protocol.md` for the full Canon Lookup Protocol.
2. Read `skills/world-notes-seeder/SKILL.md` (the base skill) and follow it IN FULL.
...
This skill WRITES canon. The RAG is only for reconciliation lookups ("does CHAR-012 already exist?", "is this thread already planted?", "does existing canon contradict this note?"). ALL writes go to the real `world/` files exactly as the base skill specifies — never treat the RAG as a write target. END every run by reminding the user to rebuild the RAG index (Help → Maintaining & Embedding the RAG, or `POST /api/rag/ingest`), since `world/` changed.
