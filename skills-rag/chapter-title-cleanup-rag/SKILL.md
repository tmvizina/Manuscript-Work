---
name: chapter-title-cleanup-rag
description: RAG-aware variant of chapter-title-cleanup. Cleans up, audits, renumbers, and standardizes chapter, part, and section titles and manuscript filenames without rewriting prose. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to chapter-title-cleanup.
---

# chapter-title-cleanup-rag

## What this is

A thin RAG-aware wrapper around `skills/chapter-title-cleanup`. Behavior, inputs, outputs, and guarantees are identical to the base skill; the only change is that canon/world lookups (who/what/when facts from `world/` memory) are answered by querying the Book Writer RAG endpoint instead of reading whole `world/` files.

This skill rarely needs canon: RAG queries are only for occasional canon checks (e.g., proper-noun spellings in titles against `world/` canon) — most runs won't need any.

## How to run

1. Read `skills-rag/_shared/canon-lookup-protocol.md` for the full Canon Lookup Protocol.
2. Read `skills/chapter-title-cleanup/SKILL.md` (the base skill) and follow it IN FULL.
