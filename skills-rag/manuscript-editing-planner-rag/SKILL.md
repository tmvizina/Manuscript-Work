...
description: RAG-aware variant of manuscript-editing-planner. Editing planner (v1) — converts manuscript review reports into a structured editing plan with book-structure recommendations, chapter-split proposals, new title suggestions, and detailed per-chapter editing plans. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to manuscript-editing-planner.
---
...
**EMPHATIC: manuscript prose being planned for edit must be read from the chapter files IN FULL — the RAG must never substitute for reading the text being worked on.**
