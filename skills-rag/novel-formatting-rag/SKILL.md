...
description: RAG-aware variant of novel-formatting. Formats manuscript files into clean, consistent novel-ready Markdown or plain text while preserving prose, chapter order, scene breaks, section headings, and audiobook readability. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to novel-formatting.
---
...
A thin RAG-aware wrapper around `skills/novel-formatting`. Behavior, inputs, outputs, and guarantees are identical to the base skill; the only change is that canon/world lookups (who/what/when facts from `world/` memory) are answered by querying the Book Writer RAG endpoint instead of reading whole `world/` files.

This skill rarely needs canon: RAG queries are only for occasional canon checks (e.g., proper-noun spellings against `world/` canon while formatting headings) — most runs won't need any.

## How to run

1. Read `skills-rag/_shared/canon-lookup-protocol.md` for the full Canon Lookup Protocol.
2. Read `skills/novel-formatting/SKILL.md` (the base skill) and follow it IN FULL.
