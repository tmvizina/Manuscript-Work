...
description: RAG-aware variant of manuscript-planner. The generation planner — converts a human-validated outline into a writer-ready generation guide (chapter targets, scene briefs, per-chapter PLANT/GROW/HARVEST thread beats, voice/continuity anchors) while preserving campaign-pending threads as plant/grow-only. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to manuscript-planner.
---
...
2. Read `skills/manuscript-planner/SKILL.md` (the base skill) and follow it IN FULL.
