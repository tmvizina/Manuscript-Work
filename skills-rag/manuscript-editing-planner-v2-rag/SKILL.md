...
description: RAG-aware variant of manuscript-editing-planner-v2. Enhanced editing planner — stable EP-NNN plan IDs referencing reviewer RV-NNN findings, dependency graph, conflict detection, per-item risk register and effort estimates, and pass scoping for focused multi-chapter plans. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to manuscript-editing-planner-v2.
---
...
**EMPHATIC: manuscript prose being planned for edit must be read from the chapter files IN FULL — the RAG must never substitute for reading the text being worked on (conflict detection between plan items requires the real passages).**
