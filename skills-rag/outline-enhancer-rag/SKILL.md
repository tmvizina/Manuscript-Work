...
description: RAG-aware variant of outline-enhancer. The front-of-pipeline dramaturg — deepens a human seed (premise, outline, beat sheet) into an enriched outline while seeding world/ memory (characters, threads, arcs with stable IDs), marking unresolved questions campaign-pending rather than inventing answers. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to outline-enhancer.
---
...
A thin RAG-aware wrapper around `skills/outline-enhancer`. Behavior, inputs, outputs, and guarantees are identical to the base skill; the only change is that canon/world lookups (who/what/when facts from `world/` memory) are answered by querying the Book Writer RAG endpoint instead of reading whole `world/` files.

## How to run

1. Read `skills-rag/_shared/canon-lookup-protocol.md` for the full Canon Lookup Protocol.
2. Read `skills/outline-enhancer/SKILL.md` (the base skill) and follow it IN FULL.
...
This skill WRITES canon. The RAG is only for reconciliation lookups ("does CHAR-012 already exist?", "is this thread already planted?", "was this arc already proposed?"). ALL writes go to the real `world/` files exactly as the base skill specifies — never treat the RAG as a write target. END every run by reminding the user to rebuild the RAG index (Help → Maintaining & Embedding the RAG, or `POST /api/rag/ingest`), since `world/` changed.
