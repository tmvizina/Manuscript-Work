---
name: story-arc-reviewer-rag
description: RAG-aware variant of story-arc-reviewer. The human-in-the-loop arc gate — turns the outline-enhancer's proposed arcs into concrete confirmation questions for the author, records the answers as canon in world/, and hands a human-validated outline to the manuscript-planner. World/canon lookups go through the Book Writer RAG endpoint to save tokens; behavior is otherwise identical to story-arc-reviewer.
---

# story-arc-reviewer-rag

## What this is

A thin RAG-aware wrapper around `skills/story-arc-reviewer`. Behavior, inputs, outputs, and guarantees are identical to the base skill — including the human checkpoint; the only change is that canon/world lookups (who/what/when facts from `world/` memory) are answered by querying the Book Writer RAG endpoint instead of reading whole `world/` files.

## How to run

1. Read `skills-rag/_shared/canon-lookup-protocol.md` for the full Canon Lookup Protocol.
2. Read `skills/story-arc-reviewer/SKILL.md` (the base skill) and follow it IN FULL.
